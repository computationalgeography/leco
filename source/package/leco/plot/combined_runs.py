...
def calculate_heterozygosity(language_profiles: np.array) -> float:
    ...
    counts_per_position = np.apply_along_axis(
        lambda x: np.bincount(
            x.astype(int), minlength=int(language_profiles.max()) + 1
        ),
        axis=0,
    ...
    normalized_h[mask] = heterozygosity_per_position[mask] / (
        1 - 1 / n_categories_per_position[mask]
    )
...
def compute_fixation_index_per_step(population: gpd.GeoDataFrame) -> dict[str, float]:
    ...
    heterozygosity_total = calculate_heterozygosity(
    ...
    mean_heterozygosity_within = sum(heterozygosity_per_language.values()) / len(
        heterozygosity_per_language
    )

    fixation_index = (
        heterozygosity_total - mean_heterozygosity_within
    ) / heterozygosity_total
